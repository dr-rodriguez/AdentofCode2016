...
rows that are all the same width; you take note of the safe tiles (.) and traps (^) in the first row
(your puzzle input).
...
def check_trap(line, i):
    ...
    if i == max_len:
        ...
        right = line[i+1]
    if (left == '^' and center == '^' and right == '.') or (left == '.' and center == '^' and right == '^') or \
            (left == '^' and center == '.' and right == '.') or (left == '.' and center == '.' and right == '^'):
        return '^'
    else:
        return '.'
...
def next_line(line):
    new_line = list('.' * len(line))
    for i, _ in enumerate(new_line):
        new_line[i] = check_trap(line, i)
    return ''.join([s for s in new_line])
...
while row < 40:
    print(my_input)
    count += my_input.count('.')
    my_input = next_line(my_input)
    row +=1
