...
For the data to not be suspiscious, it needs to have certain properties; purely random data will be detected as
...
def dragon(a):
    b = a.replace('1','2')
    b = b.replace('0', '1').replace('2','0')
    return a + '0' + b[::-1]


def checksum(x):
    temp = zip(*[x[i::2] for i in range(2)])
    return ''.join(['1' if j[0] == j[1] else '0' for j in temp])
