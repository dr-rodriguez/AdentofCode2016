...
my_input = "cxdnnyjw"
# my_input = "abc"  # test input

count = 0
answer = ''

while len(answer) < 8:
    val = md5(my_input + str(count)).hexdigest()
...
while inputs < 8:
    ...
    if val[:5] == '00000':
        ...
        try:
            loc = int(val[5])
            if loc < 8:
                if answer[loc] is None:
                    answer[loc] = val[6]
                    inputs += 1
                    print(answer)
        except ValueError:  # not a decimal number
            pass

    count += 1

print('Answer: {}'.format(''.join([s for s in answer])))
