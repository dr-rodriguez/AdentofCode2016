...
while len(my_input) > 0:
    ...
    # Extract for decompression
    newstring = my_input[x.end():x.end()+length] * times

    # Replace old string
    oldstring = my_input[0:x.end()+length]
    decompressed += my_input[0:x.start()] + newstring
    my_input = my_input.replace(oldstring, '')
    # print(decompressed)
    # print(my_input)
